...
class LXCSummaryWidget(BaseSummaryView):
    ...
    def update_data(self, data: list[str] | None) -> None:
        ...
        node = ProxmoxData.get_guest_information(node_name=data[-1], resource_type="lxc", vmid=data[1])
        ...
        memory_usage = (status_current.get("mem", 0) / status_current.get("maxmem", 1)) * 100
        ...
        swap_usage = (status_current.get("swap", 0) / status_current.get("maxswap", 1)) * 100
        ...
        boot_disk_usage = (status_current.get("disk", 0) / status_current.get("maxdisk", 1)) * 100
        ...
        self.query_one("#lxc_memory_label", Label).update(f"Memory {memory_info_label}:")
        self.query_one("#lxc_swap_label", Label).update(f"SWAP {swap_info_label}:")
        self.query_one("#lxc_boot_disk_label", Label).update(f"Boot disk {boot_disk_label}:")
        ...
        self.query_one("#lxc_node_value", Label).update(node.get("node", "N/A"))
        self.query_one("#lxc_tags_value", Label).update(node.get("tags", ""))
        self.query_one("#lxc_unprivileged_value", Label).update(
            "Yes" if status_current.get("unprivileged", 1) == 1 else "No"
        )
        self.query_one("#lxc_cpu_bar", ProgressBar).update(progress=cpu_usage)
        self.query_one("#lxc_memory_bar", ProgressBar).update(progress=memory_usage)
        self.query_one("#lxc_swap_bar", ProgressBar).update(progress=swap_usage)
        self.query_one("#lxc_boot_disk_bar", ProgressBar).update(progress=boot_disk_usage)
