...
class LXCGraphWidget(BaseGraphView):
    ...
    def update_data(self,  data: list[str] | None) -> None:
        ...
        lxc_cpu_graph = self.query_one("#lxc_cpu_graph", GraphWidget)
        lxc_memory_graph = self.query_one("#lxc_memory_graph", GraphWidget)
        lxc_network_graph = self.query_one("#lxc_network_graph", GraphWidget)
        lxc_disk_graph = self.query_one("#lxc_disk_graph", GraphWidget)

        # we do not have any data to display on graphs
        if not time_data:
            lxc_cpu_graph.clear_data()
            lxc_memory_graph.clear_data()
            lxc_network_graph.clear_data()
            lxc_disk_graph.clear_data()
            return
        ...
        lxc_cpu_graph.set_data(
            series_x1=time_data, series_x1_limit=time_data[-1], series_y1=cpuused_data, series_y1_limit=max_cpu)

        lxc_memory_graph.set_data(
            series_x1=time_data, series_x1_limit=time_data[-1], series_y1=memused_data, series_y1_limit=maxmem/1024/1024)

        lxc_network_graph.set_data(series_x1=time_data, series_x1_limit=time_data[-1], series_y1=netin_data, series_y1_limit=max_net,
                                   series_x2=time_data, series_x2_limit=time_data[-1], series_y2=netout_data, series_y2_limit=max_net)

        lxc_disk_graph.set_data(series_x1=time_data, series_x1_limit=time_data[-1], series_y1=diskread_data, series_y1_limit=max_disk,
                                series_x2=time_data, series_x2_limit=time_data[-1], series_y2=diskwrite_data, series_y2_limit=max_disk)
