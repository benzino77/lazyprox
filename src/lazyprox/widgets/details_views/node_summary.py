...
class NodeSummaryWidget(BaseSummaryView):
    ...
    def update_data(self, data: list[str] | None) -> None:
        ...
        ksm_sharing = node.get("full_status", {}).get("ksm", {}).get("shared", 0)
        ksm_sharing_str = format_bytes(ksm_sharing)

        swap_total = format_bytes(node.get("full_status", {}).get("swap", {}).get("total", 0))
        swap_free = format_bytes(node.get("full_status", {}).get("swap", {}).get("free", 0))
        swap_used = format_bytes(node.get("full_status", {}).get("swap", {}).get("used", 0))
        swap_str = f"Total: {swap_total}, Free: {swap_free}, Used: {swap_used}"
        cpu_model = node.get("full_status", {}).get("cpuinfo", {}).get("model", "N/A")
        cpu_sockets = node.get("full_status", {}).get("cpuinfo", {}).get("sockets", 0)
        cpu_cores = node.get("full_status", {}).get("cpuinfo", {}).get("cores", 0)
        kernel_version_str = node.get("full_status", {}).get("current-kernel", {}).get("release", "N/A")
        boot_mode_str = node.get("full_status", {}).get("boot-info", {}).get("mode", "N/A")
        manager_version_str = node.get("full_status", {}).get("pveversion", "N/A")

        self.query_one("#node_cpu_label", Label).update(f"CPUs({cpu_count}):")
        self.query_one("#node_memory_label", Label).update(f"Memory {memory_info}:")
        ...
        self.query_one("#node_load_average_value", Label).update(load_average_str)
        self.query_one("#node_memory_bar", ProgressBar).update(progress=memory_usage)
        ...
        self.query_one("#node_ksm_sharing_value", Label).update(ksm_sharing_str)
        self.query_one("#node_swap_value", Label).update(swap_str)
        self.query_one("#node_cpu_model_value", Label).update(cpu_model)
        self.query_one("#node_cpu_sockets_value", Label).update(str(cpu_sockets))
        self.query_one("#node_cpu_cores_value", Label).update(str(cpu_cores))
        self.query_one("#node_kernel_version_value", Label).update(kernel_version_str)
        self.query_one("#node_boot_mode_value", Label).update(boot_mode_str)
        self.query_one("#node_manager_version_value", Label).update(manager_version_str)
