...
class QemuSummaryWidget(BaseSummaryView):
    ...
    def update_data(self, data: list[str] | None) -> None:
        if not data:
            self.clear_data({"#qemu_cpu_label": "CPUs:", "#qemu_memory_label": "Memory:"})
            return

        node = ProxmoxData.get_guest_information(node_name=data[-1], resource_type="qemu", vmid=data[1])
        ...
        self.query_one("#qemu_cpu_label", Label).update(f"CPUs({cpu_count_label}):")
        self.query_one("#qemu_memory_label", Label).update(f"Memory {memory_info_label}:")
        ...
        self.query_one("#qemu_node_value", Label).update(node.get("node", "N/A"))
        self.query_one("#qemu_tags_value", Label).update(node.get("tags", ""))
        self.query_one("#qemu_cpu_bar", ProgressBar).update(progress=cpu_usage)
        self.query_one("#qemu_memory_bar", ProgressBar).update(progress=memory_usage)
        self.query_one("#qemu_host_memory_usage_value", Label).update(host_memory_usage)
